  ...
  return (
    <Fragment>
      ...
      {filteredBlog.map((blog, index)=>(
        <li key={index} className={index === filteredBlog.length - 1 ? 'borderTop end' : 'borderTop'}>
          <Link href={`/blog/${blog.slug}`}>
            <a>
              ...
              {props.isTagDisplayed && isTagDisplayed && <div className='tags'>
                {blog.tag.map((tag, index) => index === blog.tag.length - 1 ? 
